...
app.get("/accReqNotif", async (req, res) => {
  try {
    const response = await axios.get("http://localhost:8080/accReqNotif");
    res.status(200).json(response.data);
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error fetching account request notifications" });
  }
});
...
describe("Unit Testing for Enrollment Officer Dashboard Function", () => {
  describe("For /getCS", () => {
  ...
  describe("For /accReqNotif", () => {
    it("Should return the account request notification count", async () => {
      axios.get.mockResolvedValue({
        data: { studentCount: 10, empCount: 5, societyCount: 3 },
      });

      const response = await request(app).get("/accReqNotif");

      expect(response.status).toBe(200);
      expect(response.body.studentCount).toBe(10);
      expect(response.body.empCount).toBe(5);
      expect(response.body.societyCount).toBe(3);
    });

    it("Should handle errors when fetching account request notifications", async () => {
      axios.get.mockRejectedValue(
        new Error("Error fetching account request notifications")
      );
      ...
      expect(response.body.message).toBe(
        "Error fetching account request notifications"
      );
    });
  });
