    ...
    if (results.length > 0) {
      ...
      if (user.Status === "Terminated") {
        return res.status(400).json({
          message:
            "Account is no longer active. Fill out contact form to ask for reactivation",
          isLoggedIn: false,
        });
      }
...
describe("Unit Testing for the Login Function", () => {
  ...
  it("Should return a successful login for valid credentials", async () => {
    mysql
      ...
      .query.mockImplementationOnce((sql, params, callback) => {
        if (
          sql.includes("SELECT * FROM account WHERE Email = ? AND Password = ?")
        ) {
  ...
  it("Should return an error for invalid credentials", async () => {
    ...
    const response = await request(app).post("/LoginPage").send({
      email: "enrollmentofficer@cvsu.edu.ph",
      password: "enrollmentofficer",
    });
