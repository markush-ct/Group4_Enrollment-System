...
describe('SidebarMenu Component', () => {
  ...
  test('Renders sidebar menu correctly', () => {
    ...
    expect(screen.getByText(/CAVITE STATE UNIVERSITY BACOOR CITY CAMPUS/i)).toBeInTheDocument();
  ...
  test('Displays and hides "About" dropdown when clicked', async () => {
    ...
    expect(dropdownItems[1]).toHaveTextContent(/Mission, Vision, and Core Values/i);
    expect(dropdownItems[2]).toHaveTextContent(/Department of Computer Studies/i);
    expect(dropdownItems[3]).toHaveTextContent(/Computer Studies Society Officers/i);
  ...
  test('Links navigate to correct routes', () => {
    ...
    expect(screen.getByText(/History of CvSU/i)).toHaveAttribute('href', '/CvsuHistory');

    expect(screen.getByText(/Mission, Vision, and Core Values/i)).toHaveAttribute('href', '/MissionVision');

    const departmentLinks = screen.getAllByText(/Department of Computer Studies/i);
    expect(departmentLinks[1]).toHaveAttribute('href', '/DcsPage'); 

    expect(screen.getByText(/Computer Studies Society Officers/i)).toHaveAttribute('href', '/SocOff');

    fireEvent.click(screen.getByText(/Admissions/i));

    expect(screen.getByText(/Apply/i)).toHaveAttribute('href', '/Apply');
    
    expect(screen.getByText(/Enrollment FAQs/i)).toHaveAttribute('href', '/FAQS');
    
    expect(screen.getByText(/Undergraduate Programs/i)).toHaveAttribute('href', '/Undergrad');

    expect(screen.getByText(/Contact/i)).toHaveAttribute('href', '/MainPage#contact');
