...
describe('MissionVision Component', () => {
  ...
  test('Renders the MissionVision component', () => {
    ...
    const heading = screen.getByRole('heading', { name: /CAVITE STATE UNIVERSITY/i });
    expect(heading).toBeInTheDocument();

    expect(screen.getByText(/Mission, Vision and Core Values/i, { selector: 'h1' })).toBeInTheDocument();
  });
  ...
  test('Renders Mission, Hangarin ng Pamantasan, Vision, Mithiin ng Pamantasan, Quality Policy, and Core Values correctly.', async () => {
    ...
    expect(screen.getByText(/Cavite State University shall provide excellent/i)).toBeInTheDocument();
    expect(screen.getByText(/Hangarin ng Pamantasan/i)).toBeInTheDocument();
    expect(screen.getByText(/Ang Cavite State university ay makapagbigay ng mahusay, pantay at makabuluhang edukasyon/i)).toBeInTheDocument();
    ...
    expect(screen.getByText(/The premier university in historic Cavite globally/i)).toBeInTheDocument();

    expect(screen.getByText(/Mithiin ng Pamantasan/i)).toBeInTheDocument();
    expect(screen.getByText(/Ang nangungunang pamantasan sa makasaysayang Kabite/i)).toBeInTheDocument();
  ...
  test('Renders the goals of CvSU Bacoor City Campus', () => {
    ...
    expect(screen.getByText(/Goals of the CvSU Bacoor City Campus/i)).toBeInTheDocument();
    expect(screen.getByText(/In support to the Vision and Mission of the University, CvSU – Bacoor City Campus shall:/i)).toBeInTheDocument();
    expect(screen.getByText(/1. provide quality and affordable education/i)).toBeInTheDocument();
    expect(screen.getByText(/2. prepare students to meet the demands of the global market/i)).toBeInTheDocument();
    expect(screen.getByText(/3. develop innovative and scholarly researchers/i)).toBeInTheDocument();
    expect(screen.getByText(/4. and produce globally competitive graduates/i)).toBeInTheDocument();
  });
  ...
  test('Renders page footer correctly', async () => {
    ...
    expect(footers[0]).toHaveTextContent(/© Copyright.*Cavite State University.*All Rights Reserved./);
