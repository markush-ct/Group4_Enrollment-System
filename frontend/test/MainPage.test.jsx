...
expect(headerElements[0]).toBeInTheDocument(); 
  ...
  test('Renders the content section', () => {
    expect(screen.getByText('Welcome to Your Future in Technology')).toBeInTheDocument();
    expect(screen.getByText('Are you ready to shape the future and be at the forefront of innovation?')).toBeInTheDocument();
  });
  ...
  it('Renders "Join Us Now" section with right additional text', () => {
    ...
    const joinUsSubheadings = screen.getAllByText(/Taking a course in Computer Studies is a great way to prepare for a career/i);
  ...
  test('Renders the location icon and information', () => {
    ...
    expect(locationIcon).toHaveAttribute('src', '/src/assets/location-logo.svg'); 

    const addressText = screen.getByText(/Cavite State University - Bacoor Campus, Bacoor, Cavite./i);
  ...
  test('Renders the email icon and information', () => {
    ...
    expect(emailLink.closest('a')).toHaveAttribute('href', 'mailto:cvsubacoor@cvsu.edu.ph');
  });
  ...
  test('Ensures all icons are displayed correctly', () => {
    ...
    expect(iconElements.filter(icon => 
  ...
  test('Checks if the contact form handles emailjs failure correctly', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Failed to send email: Error: Email sending failed')).toBeInTheDocument();
    });
  });
  ...
  test('Renders Cavite State University logo and text', () => {
    ...
    const brandingText = screen.getByText('Cavite State University - Bacoor Campus');
  ...
  test('Renders the Facebook social icon with a link', () => {
    ...
    expect(facebookLink).toHaveAttribute('href', 'https://www.facebook.com/CvSUBacoorCityCampus');
