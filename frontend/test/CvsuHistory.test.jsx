...
describe("Unit Testing for CvSU History Page", () => {
  ...
  test("Renders Cavite State University History page correctly along with the footer", async () => {
    ...
    expect(
      screen.getByText(
        /The Cavite State University \(CvSU\) has its humble beginnings /
      )
    ).toBeInTheDocument();
    expect(
      screen.getByText(/On January 22, 1998, by virtue of Republic Act No.8468/)
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /CvSU is mandated “to provide excellent, equitable and relevant educational opportunities/
      )
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /The University is offering close to 100 curricular programs in the undergraduate and graduate levels/
      )
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /CvSU adheres to its commitment to Truth, Excellence and Service/
      )
    ).toBeInTheDocument();
